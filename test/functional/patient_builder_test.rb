...
class PatientBuilderFunctionalTest < ActionController::TestCase
  ...
  test 'UnixTime results are in seconds' do
    records_set = File.join('cqm_patients', 'CMS878v0')
    ...
    record = CQM::Patient.where(familyName: 'NoNumerator').first
  ...
  test "Components' UnixTime results are in seconds" do
    patients_set = File.join('cqm_patients', 'CMS879v0')
    users_set = File.join('users', 'base_set')
    ...
    patient = CQM::Patient.where(familyName: 'NoNumerator').first
    # clear calculated value from fixture
