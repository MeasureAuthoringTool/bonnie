describe 'InputView', ->
  ...
  describe 'CqlDateTimeView', ->

    describe 'initialization', ->

      it 'invalid by default if not allowNull  ', ->
        view = new Thorax.Views.InputCqlDateTimeView({ allowNull: false })
        view.render()
      ...
      it 'valid by default if allowNull  ', ->
        view = new Thorax.Views.InputCqlDateTimeView({ allowNull: true })
        view.render()
      ...
      it 'can start with a fully filled datetime', ->
        ...
        view = new Thorax.Views.InputCqlDateTimeView(initialValue: date)
        view.render()
    ...
    describe 'handles changes', ->
      ...
      it 'handles the user unchecking both ends, then re-enabling, using dates based on today for default', ->
        ...
        today = new Date()
        newDate = new cqm.models.CQL.DateTime(2020, today.getMonth() + 1, today.getDate(), 8, 0, 0, 0, 0)
        expect(@view.value).toEqual(newDate)
    ...
    describe 'createDefault', ->
      ...
      it 'uses defaultYear when provided', ->
        date = new cqm.models.CQL.DateTime(2012, 2, 23, 8, 15, 0, 0, 0)
        view = new Thorax.Views.InputCqlDateTimeView(initialValue: date, defaultYear: 2019)
