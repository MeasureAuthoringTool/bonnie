...
# Bonnie doesn't support choice types in primary code path
  ...
  @stringifyCompositeValue: (compositeValue, codeSystemMap) ->
    attrs = []
    cqm.models[compositeValue.getTypeName()].fieldInfo.forEach (fieldInfo) =>
      ...
      if skip || fieldName is 'id' || fieldInfo.isArray
        ...
        fieldValue = compositeValue[fieldName]
        if !fieldValue?
          # Don't display nulls
        else if cqm.models.SampledData.isSampledData(fieldValue)
          attrs.push("#{fieldName}: [#{@stringifyType(fieldValue, codeSystemMap)}]")
        else
          attrs.push("#{fieldName}: #{@stringifyType(fieldValue, codeSystemMap)}")
    return attrs.join(" | ")
...
#      },
  ...
  # 2nd level entries are properties with custom metadata
  @COMPOSITE_TYPES:
    EncounterLocation:
      location:
        referenceTypes: ['Location']
      # Skip UI for the following fields
      physicalType:
        skip: true
    EncounterDiagnosis:
      condition:
