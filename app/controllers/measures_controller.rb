  ...
  def create
    ...
    begin
      ...
      if e.is_a? Measures::VSACException
        ...
        operator_error = true
        flash[:error] = {title: "Error Loading Measure", summary: "The measure could not be loaded #{e.message}", body:"There may be an error in the CQL logic."}
      else
        flash[:error] = {title: "Error Loading Measure", summary: "The measure could not be loaded #{e.message}.", body: "Bonnie has encountered an error while trying to load the measure."}
      end
