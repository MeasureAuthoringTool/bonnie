module CQM
  ...
  def update_expected_value_structure!(measure)
    ...
    self.expectedValues.reject! do |expected_value_set|
    ...
    patient_population_count = self.expectedValues.count { |expected_value_set| expected_value_set[:measure_id] == measure.hqmf_set_id }
    ...
    if patient_population_count < measure_population_count
      (patient_population_count..measure_population_count-1).each do |index|
        new_expectedValues = {"measure_id" => measure.hqmf_set_id, "population_index" => index}
        # yield info about this addition
        yield :population_set_addition, :missing_population, new_expectedValues.deep_dup if block_given?
        self.expectedValues << new_expectedValues
      end
    end

    # ensure there's the correct number of populations for each population set
    self.expectedValues.each do |expected_value_set|
