...
#         if value['type'] == 'CD'
#           oid = value['code_list_id']
#           codes = value['codes'] || Coded.select_codes(oid, value_sets)
#           entry.values << CodedResultValue.new({codes: codes, description: (value_sets[oid] || {})["display_name"]})
#         else
#           range = HQMF::Range.from_json('low' => {'value' => value['value'], 'unit' => value['unit']}))
#           entry.values <<  PhysicalQuantityResultValue.new(range.format)
#         end
#       end
#     end
