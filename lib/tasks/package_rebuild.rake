namespace :bonnie do
  namespace :cql do
    desc %{Rebuilds the JSON and XML ELM for a MAT package or extracted directory. Saves as a new file with '_rebuilt' appened to
      the file name if a zip file is provided.
  
      $ bundle exec rake bonnie:cql:add_json_to_package[path/to/package.zip]

      or

      $ bundle exec rake bonnie:cql:add_json_to_package[path/to/package_dir]
      ...
      task :rebuild_package, [:input_package_or_dir_path] => [:environment] do |t, args|
        ...
        if input_package_or_dir_path.directory?
          ...
          Zip::ZipFile.open(input_package_or_dir_path) do |zip_file|
            ...
            zip_file.glob(File.join('**','**.cql')).select {|x| !x.name.starts_with?('__MACOSX') }.each do |cql_file_path|
              zip_file.extract(cql_file_path, File.join(measure_path, File.basename(cql_file_path.name)))
            end
            zip_file.glob(File.join('**','**.xml')).select {|x| !x.name.starts_with?('__MACOSX') }.each do |xml_file_path|
        ...
        old_elm_xml_filepaths = Dir.glob(File.join(measure_path, "*.xml")).select { |x| x != hqmf_file_path }
        ...
        output_package_path = if input_package_or_dir_path.directory?
          Pathname.new(File.join(input_package_or_dir_path.dirname, input_package_or_dir_path.basename.to_s + "_rebuilt.zip"))
        else 
          Pathname.new(File.join(input_package_or_dir_path.dirname, input_package_or_dir_path.basename('.zip').to_s + "_rebuilt.zip"))
        end
